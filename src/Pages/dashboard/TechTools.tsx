...
import type { I18nContextValue, TechTool, ToolFormData } from "../../types";
import { errMessage } from "../../types";

const typeOptions = (t: I18nContextValue["t"]) => [
  { value: "Main" as const, label: t("dashboard.toolTypeMain") },
  { value: "Other" as const, label: t("dashboard.toolTypeOther") },
];

type ToolFormSubmit = ToolFormData & { imageFile: File | null; imageLightFile: File | null };
...
const ToolForm = ({
  ...
  initial?: TechTool | null;
  onSubmit: (form: ToolFormSubmit) => void;
  onCancel: () => void;
  submitLabel: string;
  uploading: boolean;
}) => {
  ...
  const [imageLightFile, setImageLightFile] = useState<File | null>(null);
  const [previewImage, setPreviewImage] = useState<string | null>(initial?.image || null);
  const [previewImageLight, setPreviewImageLight] = useState<string | null>(initial?.image_light || null);

  const set = (key: keyof ToolFormData) => (e: ChangeEvent<HTMLInputElement>) =>
    setForm((f) => ({ ...f, [key]: e.target.value }));

  const handleImageFile = (e: ChangeEvent<HTMLInputElement>, target: "image" | "light") => {
...
const ToolCard = ({
  ...
  index: number;
  total: number;
  onDelete: (tool: TechTool) => void;
  onEdit: (tool: TechTool) => void;
  onToggleVisibility: (tool: TechTool) => void;
...
export default function TechTools() {
  ...
  const [filter, setFilter] = useState<"all" | "Main" | "Other">("all");
  const [showCreate, setShowCreate] = useState(false);
  const [editTool, setEditTool] = useState<TechTool | null>(null);
  ...
  const fetchTools = useCallback(async () => {
    ...
    setTools((data || []) as unknown as TechTool[]);
    setLoading(false);
  }, []);
  ...
  const handleCreate = async (form: ToolFormSubmit) => {
    try {
      setUploading(true);
      ...
      Swal.fire({
        ...
        text: errMessage(error) || t("dashboard.toolCreateFailed"),
        background: "var(--bg-secondary)",
        color: "var(--text-primary)",
      });
    } finally {
  ...
  const handleEdit = async (form: ToolFormSubmit) => {
    try {
      setUploading(true);
      ...
      Swal.fire({
        icon: "error",
        title: t("common.errorTitle"),
        text: errMessage(error) || t("dashboard.toolUpdateFailed"),
        background: "var(--bg-secondary)",
        color: "var(--text-primary)",
      });
    } finally {
  ...
  const deleteTool = async (tool: TechTool) => {
  ...
  const handleMove = (toolId: string) => (targetIndex: number) => {
  ...
  const handleToggleVisibility = async (tool: TechTool) => {
  ...
  const filterOptions = useMemo(
    () => [
      { key: "all" as const, label: `${t("dashboard.filterAll")} (${tools.length})` },
      { key: "Main" as const, label: `${t("dashboard.toolTypeMain")} (${mainCount})` },
      { key: "Other" as const, label: `${t("dashboard.toolTypeOther")} (${otherCount})` },
    ],
