...
const leadingEllipsis = (url: string): string => {
  if (!url) return url;
...
export default function PersonalInfo() {
  ...
  const [totalProjects, setTotalProjects] = useState<string>(() => loadLocal("totalProjects", ""));
  const [yearsExp, setYearsExp] = useState<string>(() => loadLocal("yearsExperience", ""));
  const [showYearsExp, setShowYearsExp] = useState<boolean>(() => loadLocal("showYearsExperience", false));
  ...
  const [address, setAddress] = useState<string>(() => loadLocal("address", hardcodedDefaults.address));
  const [addressAr, setAddressAr] = useState<string>(() => loadLocal("addressAr", hardcodedDefaults.addressAr));
  const [phone, setPhone] = useState<string>(() => loadLocal("phone", hardcodedDefaults.phone));
  const [email, setEmail] = useState<string>(() => loadLocal("email", hardcodedDefaults.email));
  const [socialLinks, setSocialLinks] = useState<SocialLink[]>(() => {
    const saved = loadLocal<SocialLink[] | null>("socialLinks", null);
      ...
      try {
        ...
        if (data) {
          ...
          if (map.socialLinks !== undefined) {
            const parsed = parseJSON<SocialLink[]>(map.socialLinks, []);
