...
export default function Comments() {
  ...
  const fetchComments = async () => {
    ...
    if (settingsData) {
      setIsFrozen((settingsData as unknown as AppSetting).value === "true");
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start sm:items-center justify-between gap-4 flex-wrap">
        ...
        <div className="grid grid-cols-2 gap-3 sm:min-w-[280px]">
          {[
            { value: "all" as const, label: t("dashboard.filterAll"), count: comments.length },
            { value: "pinned" as const, label: t("dashboard.filterPinned"), count: pinnedCount },
          ].map((tab) => (
            <button
              key={tab.value}
