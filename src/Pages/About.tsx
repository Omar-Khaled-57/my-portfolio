...
const AboutPage = () => {
  ...
  const countAccessible = (projects: Project[]) =>
  ...
  const readLocalNum = (key: string, fallback: number): number => {
    try {
  ...
  const [yearsExpValue, setYearsExpValue] = useState<number>(() => readLocalNum("personalInfo_yearsExperience", 0));
  const [manualTotalProjects, setManualTotalProjects] = useState<number>(() => readLocalNum("personalInfo_totalProjects", 26));
  ...
  const [stats, setStats] = useState<AboutStats>(() => {
    ...
    let certs: unknown[] = [];
    try { projects = JSON.parse(localStorage.getItem("projects") || "[]") as Project[]; } catch {}
    try { certs = JSON.parse(localStorage.getItem("certificates") || "[]") as unknown[]; } catch {}
    return { totalCertificates: certs.length, accessibleProjects: countAccessible(projects) };
  });

  const cacheLocally = (fields?: Partial<Record<string, string | undefined>>) => {
    try {
    ...
    const handleStorage = (e: StorageEvent) => {
      if (e.key?.startsWith("personalInfo_")) {
        try {
          ...
          if (key === "totalProjects") setManualTotalProjects(JSON.parse(e.newValue || "0"));
          if (key === "yearsExperience") setYearsExpValue(JSON.parse(e.newValue || "0"));
  ...
  // Memoized stats data
  const showProjectsNotice = useCallback(() => {
    void import("sweetalert2").then((mod) => {
      mod.default.fire({
        icon: "info",
        title: t("about.totalProjects"),
        text: t("about.projectsPrivacyNotice"),
  ...
  const statsData = useMemo(() => {
    ...
    return base;
  }, [manualTotalProjects, totalCertificates, accessibleProjects, showYearsExp, yearsExpValue, showProjectsNotice, t]);
