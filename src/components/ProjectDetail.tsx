...
const ToolIconBadge = ({ tool, theme, label }: ToolIconBadgeProps) => {
  return (
    <div className="group relative overflow-hidden px-3 py-2 md:px-4 md:py-2.5 rounded-xl cursor-default transition-all duration-300 hover:scale-105 hover:shadow-accent-primary/10 border border-primary"
      style={{
        background: 'rgba(var(--color-accent-primary-rgb, 99 102 241) / 0.1)',
...
const ProjectStats = ({ project, t }: ProjectStatsProps) => {
...
const ProjectDetails = () => {
  ...
  const [dataLoaded, setDataLoaded] = useState(() => {
    try {
      return (JSON.parse(localStorage.getItem("projects") || "[]") as unknown[]).length > 0;
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    const storedProjects = ((JSON.parse(localStorage.getItem("projects") || "[]") as Project[])).filter(
    ...
    if (projectId) {
      selectedProject = source.find((p) => p.id === projectId) || null;
    }
    if (!selectedProject) {
