...
const findSocialUrl = (links: SocialLink[] | null | undefined, platform: string) => {
    const match = links?.find(l => l.platform?.toLowerCase() === platform.toLowerCase());
    return match?.url || '';
};

const loadDBString = (key: string, fallback: string) => {
    try {
        const val = localStorage.getItem("personalInfo_" + key);
        return val !== null ? JSON.parse(val) as string : fallback;
    } catch { return fallback; }
};

const loadDBLinks = (): SocialLink[] => {
    try {
        const val = localStorage.getItem("personalInfo_socialLinks");
        return val ? JSON.parse(val) as SocialLink[] : [];
    } catch { return []; }
};
...
interface CVSkillGroup { label: string; items: string[] }
interface CVProject { title: string; tag?: string; items: string[] }
interface CVSectionTitles {
    contact: string; skills: string; languages: string; courses: string;
    summary: string; education: string; internship: string; achievements: string; projects: string;
}
interface CVDataLang {
    name: string;
    title: string;
    sections: CVSectionTitles;
    contact: CVContact;
    skills: { frontend: CVSkillGroup; backend: CVSkillGroup };
    languages: string[];
    courses: string[];
    summary: string;
    education: { university: string; faculty: string; year: string };
    internship: { title: string; period: string; items: string[] };
    achievement: { title: string; period: string; items: string[] };
    projects: CVProject[];
}

const cvData: { en: CVDataLang; ar: CVDataLang } = {
    en: {
                    ...
                    <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
                        ...
                        <motion.section variants={itemVariants}>
                            ...
                            <p className="text-[13px] text-secondary leading-relaxed">
                                {d.languages.map((l, i) => (
                                    <Fragment key={l}>{l}{i < d.languages.length - 1 && <br />}</Fragment>
                                ))}
                            </p>
                        </motion.section>
